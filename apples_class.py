...
class Apple:
    ...
    def __init__(self, id, size, screenwidth, screenheight):
        ...
        self.generate(screenwidth, screenheight)
    ...
    def generate(self, screenwidth, screenheight):
        used = False
        while not used:
            hit = False
            self.cords_center.x = random.randint(0, screenwidth)
            self.cords_center.y = random.randint(0, screenheight)
            self.hitbox()
            for i in Apple.iterate_all_instances():
                if self.ishit(i.cords_center) and self.border_reached(screenwidth, screenheight):
                    hit = True
    ...
    def border_reached(self, screenwidth, screenheight):
        return (
                self.cords_upper_left.x <= 20 or
                self.cords_lower_right.x >= screenwidth - 20 or
                self.cords_upper_left.y <= 20 or
                self.cords_lower_right.y >= screenheight - 20
        )
